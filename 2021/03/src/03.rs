...
pub fn part_2(input: aoc::Input) -> impl ToString {
    let ogr = get_raiting(input.as_lines(), 0, SystemRaiting::OxygenGeneratorRating)
        .get(0)
        .unwrap()
        .clone();
    let csr = get_raiting(input.as_lines(), 0, SystemRaiting::CO2ScrubberRating)
        .get(0)
        .unwrap()
        .clone();
...
fn get_raiting(vec: &[&str], index: usize, system_type: SystemRaiting) -> Vec<String> {
    ...
    if accepted.len() == 1 {
        ...
        // TODO fix
        //get_raiting(accepted.as_slice(), index + 1, system_type)
        !todo!("needs fix");
    }
}
