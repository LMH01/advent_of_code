...
pub fn part_1(input: aoc::Input) -> impl ToString {
    let mut container = TowerContainer::from_input(input.as_lines());
    let tower_height = container.max_height;
    for (index, line) in input.as_lines().iter().enumerate() {
...
fn tower_stats(content: &[&str]) -> (u32, u32) {
    ...
    let line = content.get((tower_line - 1) as usize).unwrap();
    for c in line.chars() {
        if c.is_ascii_digit() {
...
fn parse_tower(content: &[&str], max_height: u32, tower_number: u32) -> Vec<char> {
